...
import { useNavigate } from "react-router-dom"; // Đã có sẵn trong code của bạn
...
const AdminBillList = () => {
    ...
    useEffect(() => {
        ...
        } else if (activeTab === "all" && selectedStatus) {
            result = result.filter(bill => bill.status === selectedStatus);
        }
    ...
    const handleCreateBooking = () => {
        // Điều hướng sang trang /create-fixed-booking
                                ...
                                <div className="flex-1">
                                    <div className="flex space-x-2 mb-1">
                                        ...
                                        {bill.type && (
                                            ...
                                            >
                                                {bill.type === "fixed" ? "Đơn cố định" :
                                                    bill.type === "monthly" ? "Đơn tháng" :
                                                        bill.type === "daily" ? "Đơn ngày" :
                                                            bill.type}
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className="text-sm text-gray-600">
                                        Mã đơn: {bill.bookingCode}
                                    </div>
                                    <div className="text-sm text-gray-600">
                                        Chi tiết: ({new Date(bill.createdAt).toLocaleDateString("vi-VN")});{" "}
                                        {bill.courtTime}
                                    </div>
                                    <div className="text-sm text-gray-600">
                    ...
                    <div className="fixed inset-0 flex items-center justify-center p-4">
                        <Dialog.Panel className="bg-white p-6 rounded-lg max-w-lg w-full">
                            ...
                            <div className="mt-4 space-y-2">
                                <p><strong>Tên:</strong> {selectedBill.userName || "N/A"}</p>
                                <p><strong>Mã đơn:</strong> {selectedBill.bookingCode}</p>
                                <p>
                                    <strong>Chi tiết sân:</strong>{" "}
                                    {selectedBill.courtTime}
                                </p>
                                <p>
                                ...
                                <p>
                                    ...
                                    >
                                        {selectedBill.type === "fixed" ? "Đơn cố định" :
                                            selectedBill.type === "monthly" ? "Đơn tháng" :
                                                selectedBill.type === "daily" ? "Đơn ngày" :
                                                    selectedBill.type}
                                    </span>
                                </p>
                                <p>
                                    <strong>Trung tâm:</strong>{" "}
                                    {selectedBill.centerName || "N/A"}
                                </p>
                                <p>
                                ...
                                <p><strong>Ghi chú:</strong> {selectedBill.note || "Không có ghi chú"}</p>
                                <p><strong>Tổng giá tiền:</strong> {selectedBill.totalAmount.toLocaleString("vi-VN")} VNĐ</p>
                                <p>
                                    <strong>Ảnh thanh toán:</strong>{" "}
                                    {selectedBill.paymentImage ? (
                                        <BillImage bill={selectedBill} />
                                    ) : (
                                        "Không có ảnh"
                                    )}
                                </p>
